...
export const initWordLists = async () => {
  const [{ VALID_GUESSES }, { NORMAL_WORDS }, { HARD_WORDS }] = await Promise.all([
...
export const getRandomWord = (length: number = 5, hardMode: boolean = false) => {
  const pool = (hardMode ? HARD_WORDS_ARR : NORMAL_WORDS_ARR).filter((w) => w.length === length)
